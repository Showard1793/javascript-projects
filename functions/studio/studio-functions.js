...
function reverseCharacters(str){
  ...
  console.log(reverseCharacters("Hey There!"));
  ...
  function reverseDigits(str){
    let arr = []
      ...
      let backwardsDigit = str.split("").reverse().join("");
      return backwardsDigit;
  }
  ...
  let arrayTest1 = ['apple', 'potato', 'Capitalized Words', 'At','Ma'];
  ...
  function getFirst3Chars(str){
    if (str.length > 3){
       ...
       phrase = console.log(`We put the ${newStr} in the ${str}`)
    }
  }
console.log(getFirst3Chars(str))
  ...
  function logAreaOfRectangle(length, width) {
    if (width === undefined) {
        width = length; // If only one argument is passed, assume it is a square
    }
    let area = length * width;
    let message = `The area is ${area} cm^2.`;
    return message;
}

// Example usage
console.log(logAreaOfRectangle(12,13));
